...
export function combineWhen(
  ...
  if (kind === "errand") {
    if (time.kind === "range") {
      return { ok: false, field: "time", error: "A task takes a deadline, not a time range. Try 'by 5pm' or a due date." };
    }
    if (time.kind === "start") {
    ...
    if (time.kind === "none") {
      // A Date cell with no time is due at the end of that day; no date is an anytime task.
  ...
  // shift
  if (time.kind === "dueBy") {
    return { ok: false, field: "time", error: "A shift takes a time range. A 'by ...' deadline is for tasks." };
  }
