...
describe("combineWhen", () => {
  ...
  test("frog 'by Sat 10am' resolves the weekday inside the event window", () => {
    const r = combineWhen("errand", null, { kind: "dueBy", dateText: "Sat", time: 600 }, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  test("frog 'by 3:00 PM' without a date uses the event's first day", () => {
    const r = combineWhen("errand", null, { kind: "dueBy", dateText: null, time: 900 }, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  test("frog with a plain range is an error (frogs take deadlines)", () => {
    const r = combineWhen("errand", null, { kind: "range", start: 600, end: 780 }, ctx);
  ...
  test("shift with a 'by …' deadline is an error (deadlines are for frogs)", () => {
    ...
    expect(r).toEqual({ ok: false, field: "time", error: "A shift takes a time range. A 'by ...' deadline is for tasks." });
  });
  test("frog with no time cell at all is fine — an anytime frog", () => {
    const r = combineWhen("errand", null, { kind: "none" }, ctx);
  ...
  test("frog with a single start time is rejected like a range", () => {
    const r = combineWhen("errand", null, { kind: "start", start: 600 }, ctx);
    expect(r.ok).toBe(false);
  });
  test("frog with a date and a bare time is a deadline at that date and time", () => {
    ...
    const r = combineWhen("errand", { year: 2026, month: 7, day: 25 }, { kind: "start", start: 1020 }, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  test("frog with a bare time but no day asks for a due date, no em dash", () => {
    const r = combineWhen("errand", null, { kind: "start", start: 1020 }, ctx);
    expect(r.ok).toBe(false);
    if (r.ok) return;
  ...
  test("frog with a date and no time is due at the end of that day", () => {
    const r = combineWhen("errand", { year: 2026, month: 7, day: 25 }, { kind: "none" }, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.value.dueBy?.toISOString()).toBe("2026-07-26T06:59:00.000Z"); // 11:59 PM PDT
  });
  test("frog error messages carry no em dash", () => {
    const range = combineWhen("errand", null, { kind: "range", start: 600, end: 780 }, ctx);
